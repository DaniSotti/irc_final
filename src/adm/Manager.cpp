...
void Manager::setPassword(std::string pass) {
	_password = pass; // password coming from argv[2]
}

int Manager::setPassword(std::string pass, Client &client) {
	if (_password != pass) {
		std::string errMsg = formatMessage(client, "464") + " : Password incorrect";
		sendIrcMessage(client.getId(), errMsg);
		return 1;
	}
	client.setHasPassword(true); // precisa criar este método/flag no Client
	return 0;
}
...
// int Manager::setPassword(std::string pass, Client &client) {
// 	if (_password != pass) {
// 		sendIrcMessage(client.getId(), formatMessage(client) + ":server 464 * :Password incorrect");
// 		return 1;
// 	}
// 	return 0;
// }
